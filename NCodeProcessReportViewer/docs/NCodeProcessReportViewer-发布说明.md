...
| 7 | 运行日志 | 本次处理事件时间线（时间/级别/事件/消息），error 红、warning 橙突出，可按事件类型筛选；底部显示完整磁盘日志路径（WP-C6） |
...
- 测试基线：**10 项全部通过**（conda Python 3.8 / 3.8.19，约 0.4 秒），覆盖文件发现、报告加载校验、汇总/问题/统计行生成、运行日志条目容错与筛选、第 12 节元信息与目标列渲染、悬停浮窗截断判定、窗口几何、表格无横向溢出。
