...
| 9 | 界面 | 1290×720 默认 / 1160×640 最小；左栏报告+文件明细（含「目标」列与程序名来源标记，第 12 节）、右栏 Notebook 六页；全部表格/文本区纵横滚动 |
| 10 | 悬停浮窗 | 表格单元格内容被列宽截断时，悬停自动浮窗显示完整内容（5 张表格通用） |

## 三、测试与验证

- 测试基线：**10 项全部通过**（conda Python 3.8 / 3.8.19，以 `python -m unittest discover -s tests -v` 输出为准），覆盖文件发现、报告加载校验、汇总/问题/统计行生成、运行日志条目容错与筛选、第 12 节元信息与目标列渲染、悬停浮窗截断判定、窗口几何、表格无横向溢出。
