...
- 测试基线：**15 项全部通过**（conda Python 3.8 / 3.8.19，以 `python -m unittest discover -s tests -v` 输出为准），覆盖文件发现、报告加载校验、汇总/问题/统计行生成、运行日志条目容错与筛选、运行日志详情预览区、程序列表联动过滤、默认页签断言、柱状图数值容错、第 12 节元信息与目标列渲染、悬停浮窗截断判定、窗口几何、表格无横向溢出。
