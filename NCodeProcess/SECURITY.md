...
> **维护说明**：本文件纳入版本管理，是发布产物的安全加固说明。构建脚本 `build_portable.ps1`、PyInstaller 配置 `NCodeProcess.spec` 或运行时加固钩子 `security_runtime_hook.py` 发生变更后，应同步核对并更新本文件对应章节（纳入「所有更改必须完善可能受影响的文档」约定）。
