...
- 测试基线：`NCodeProcess-测试指南.md`（159 项）、`NCodeProcessReportViewer-测试指南.md`（6 项）
...
| WP-07 | FR-07.2/07.3 校验缺口 | 中 | 主 | `core.py` | 无 | 新增刀具号一致性、孤立坐标/进给/转速、互斥 G/M 检查并有测试锁定 |
...
Expected: 159 项全部通过（新用例计入后为 160 项，测试指南基线需同步更新）。
...
- Modify: `NCodeProcess/docs/NCodeProcess-测试指南.md`（测试基线 159 → 实际数）
...
5. **测试基线**：主工具 159 项、查看器 6 项为基线；每个 WP 完成后同步更新测试指南与需求文档第 15 节。
